...
// f'''(x) from (14) [2次精度]
double dfunc6(double x, double dx){ 
  double y;
  y = (func(x+2.0*dx) - 2.0*func(x+dx) + 2.0*func(x-dx) - func(x-2.0*dx)) / (2.0*dx*dx*dx) ;
  return(y);
}

// f''''(x) from (15) [2次精度]
double dfunc7(double x, double dx){ 
  double y;
  y = (func(x+2.0*dx) - 4.0*func(x+dx) + 6.0*func(x) - 4.0*func(x-dx) + func(x-2.0*dx)) / (dx*dx*dx*dx) ;
  return(y);
}
...
int main(void){
  ...
  for(x=0.0;x<=2.0*M_PI;x=x + 2.0*M_PI / 100.0){ // 0 <= x <= 2piで100分割
    ...
    printf("%5.3f %8.5f %8.5f %8.5f %8.5f %8.5f\n", x, func(x), dfunc4(x,dx), dfunc5(x,dx), dfunc6(x,dx), dfunc7(x,dx)); // x, f(x), 1次のf'(x), 2次のf'(x), 4次のf'(x)を出力
